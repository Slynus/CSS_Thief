...
function i(t, e) { return function(t, e, i) { const n = (t || []).length; if (n > 2) { const s = []; for (let e = 0; e < n - 1; e++) s.push(...c(t[e][0], t[e][1], t[e + 1][0], t[e + 1][1], i)); return e && s.push(...c(t[n - 1][0], t[n - 1][1], t[0][0], t[0][1], i)), { type: "path", ops: s }; } return 2 === n ? s(t[0][0], t[0][1], t[1][0], t[1][1], i) : { type: "path", ops: [] }; }(t, !0, e); }
...
function o(t, e, s, i, n) {
    return function(t, e, s, i) {
        const [n, o] = f(i.increment, t, e, i.rx, i.ry, 1, i.increment * h(.1, h(.4, 1, s), s), s);
        let r = l(n, null, s);
        if (!s.disableMultiStroke) {
            const [n] = f(i.increment, t, e, i.rx, i.ry, 1.5, 0, s), o = l(n, null, s);
            r = r.concat(o);
        }
        ...
        return r += a(r * c, s), h += a(h * c, s), { increment: o, rx: r, ry: h };
    }(s, i, n)).opset;
}
...
function u(t, e, s, i, n, o, h) {
    const c = Math.pow(t - s, 2) + Math.pow(e - i, 2),
        u = Math.sqrt(c);
    let l = 1;
    l = u < 200 ? 1 : u > 500 ? .4 : -.0016668 * u + 1.233334;
    let f = n.maxRandomnessOffset || 0;
    f * f * 100 > c && (f = u / 10);
    const g = f / 2,
        d = .2 + .2 * r(n);
    ...
    const m = [],
        w = () => a(g, n, l),
        v = () => a(f, n, l);
    return o && (h ? m.push({ op: "move", data: [t + w(), e + w()] }) : m.push({ op: "move", data: [t + a(f, n, l), e + a(f, n, l)] })), h ? m.push({ op: "bcurveTo", data: [p + t + (s - t) * d + w(), _ + e + (i - e) * d + w(), p + t + 2 * (s - t) * d + w(), _ + e + 2 * (i - e) * d + w(), s + w(), i + w()] }) : m.push({ op: "bcurveTo", data: [p + t + (s - t) * d + v(), _ + e + (i - e) * d + v(), p + t + 2 * (s - t) * d + v(), _ + e + 2 * (i - e) * d + v(), s + v(), i + v()] }), m;
}
...
function l(t, e, s) {
    ...
    if (i > 3) {
        ...
        for (let e = 1; e + 2 < i; e++) {
            const s = t[e];
            o[0] = [s[0], s[1]], o[1] = [s[0] + (r * t[e + 1][0] - r * t[e - 1][0]) / 6, s[1] + (r * t[e + 1][1] - r * t[e - 1][1]) / 6], o[2] = [t[e + 1][0] + (r * t[e][0] - r * t[e + 2][0]) / 6, t[e + 1][1] + (r * t[e][1] - r * t[e + 2][1]) / 6], o[3] = [t[e + 1][0], t[e + 1][1]], n.push({ op: "bcurveTo", data: [o[1][0], o[1][1], o[2][0], o[2][1], o[3][0], o[3][1]] });
        }
        if (e && 2 === e.length) {
            const t = s.maxRandomnessOffset;
            n.push({ op: "lineTo", data: [e[0] + a(t, s), e[1] + a(t, s)] });
        }
    } else 3 === i ? (n.push({ op: "move", data: [t[1][0], t[1][1]] }), n.push({ op: "bcurveTo", data: [t[1][0], t[1][1], t[2][0], t[2][1], t[2][0], t[2][1]] })) : 2 === i && n.push(...c(t[0][0], t[0][1], t[1][0], t[1][1], s));
    return n;
}

function f(t, e, s, i, n, o, r, h) {
    const c = [],
        u = [],
        l = a(.5, h) - Math.PI / 2;
    u.push([a(o, h) + e + .9 * i * Math.cos(l - t), a(o, h) + s + .9 * n * Math.sin(l - t)]);
    for (let r = l; r < 2 * Math.PI + l - .01; r += t) {
        const t = [a(o, h) + e + i * Math.cos(r), a(o, h) + s + n * Math.sin(r)];
        c.push(t), u.push(t);
    }
    return u.push([a(o, h) + e + i * Math.cos(l + 2 * Math.PI + .5 * r), a(o, h) + s + n * Math.sin(l + 2 * Math.PI + .5 * r)]), u.push([a(o, h) + e + .98 * i * Math.cos(l + r), a(o, h) + s + .98 * n * Math.sin(l + r)]), u.push([a(o, h) + e + .9 * i * Math.cos(l + .5 * r), a(o, h) + s + .9 * n * Math.sin(l + .5 * r)]), [u, c];
}
const g = { maxRandomnessOffset: 2, roughness: 2.5, bowing: 1, stroke: "#000", strokeWidth: 1.5, curveTightness: 0, curveFitting: .95, curveStepCount: 9, fillStyle: "hachure", fillWeight: -1, hachureAngle: -41, hachureGap: -1, dashOffset: -1, dashGap: -1, zigzagOffset: -1, seed: 0, combineNestedSvgPaths: !1, disableMultiStroke: !1, disableMultiStrokeFill: !1 };

function d(t, e) {
    const s = JSON.parse(JSON.stringify(g));
    switch (t) {
        case "highlight":
            s.roughness = 3, s.disableMultiStroke = !0;
            break;
        case "single":
...
function p(e, i, r, h, a) {
    const c = [];
    let u = r.strokeWidth || 2;
    const l = function(t) {
            const e = t.padding;
            if (e || 0 === e) {
        ...
        case "underline":
            {
                const t = d("single", a),
                    e = i.y + i.h + l[2];
                for (let n = 0; n < g; n++) n % 2 ? c.push(s(i.x + i.w, e, i.x, e, t)) : c.push(s(i.x, e, i.x + i.w, e, t));
                break;
            }
        case "strike-through":
            {
        ...
        case "box":
            {
                const t = d("single", a),
                    ...
                    o = i.w + (l[1] + l[3]),
                    r = i.h + (l[0] + l[2]);
                for (let i = 0; i < g; i++) c.push(n(e, s, o, r, t));
                break;
            }
        case "crossed-off":
            {
                const t = d("single", a),
                    ...
                    o = e + i.w,
                    r = n + i.h;
                for (let i = 0; i < g; i++) i % 2 ? c.push(s(o, r, e, n, t)) : c.push(s(e, n, o, r, t));
                for (let i = 0; i < g; i++) i % 2 ? c.push(s(e, r, o, n, t)) : c.push(s(o, n, e, r, t));
                break;
            }
        case "circle":
            {
                const t = d("single", a),
                    ...
                    u = Math.floor(g / 2),
                    f = g - 2 * u;
                for (let t = 0; t < u; t++) c.push(o(r, h, s, n, e));
                for (let e = 0; e < f; e++) c.push(o(r, h, s, n, t));
                break;
            }
        case "highlight":
            { const t = d("highlight", a);u = .95 * i.h; const e = i.y + i.h / 2; for (let n = 0; n < g; n++) n % 2 ? c.push(s(i.x + i.w, e, i.x, e, t)) : c.push(s(i.x, e, i.x + i.w, e, t)); break; }
    }
    if (c.length) {
        const s = function(t) {
                const e = [];
            ...
            n = [];
        let o = 0;
        const a = 0 === r.animationDuration ? 0 : r.animationDuration || 800,
            l = (0 === r.animationDelay ? 0 : r.animationDelay || 0) + (h || 0);
        for (const h of s) {
            const s = document.createElementNS(t, "path");
            if (s.setAttribute("d", h), s.setAttribute("fill", "none"), s.setAttribute("stroke", r.color || "currentColor"), s.setAttribute("stroke-width", "" + u), f) {
        ...
        if (f) {
            ...
            for (let e = 0; e < n.length; e++) {
                const s = n[e],
                    ...
                    c = l + t,
                    u = s.style;
                u.strokeDashoffset = "" + r, u.strokeDasharray = "" + r, u.animation = `rough-notation-dash ${h}ms ease-out ${c}ms forwards`, t += h;
            }
        }
    }
}
class _ {
    constructor(t, e) {
        this._state = "unattached", this._resizing = !1, this._seed = Math.floor(Math.random() * 2 ** 31), this._animationGroupDelay = 0, this._resizeListener = () => {
            this._resizing || (this._resizing = !0, setTimeout(() => {
                if (this._resizing = !1, "showing" === this._state) {
                    const t = this.size();
                    t && this.hasRectChanged(t) && this.show();
                }
            }, 400));
        }, this._e = t, this._config = e, this.attach();
    }
    ...
    set animationDelay(t) { this._config.animationDelay = t; }
    ...
    attachListeners() {
        this.detachListeners(), window.addEventListener("resize", this._resizeListener, { passive: !0 }), !this._ro && "ResizeObserver" in window && (this._ro = new window.ResizeObserver(t => {
            for (const e of t) {
                let t = !0;
                if (e.contentRect) {
                    const s = this.sizeFor(e.contentRect);
                    s && !this.hasRectChanged(s) && (t = !1);
                }
                t && this._resizeListener();
            }
        })), this._ro && this._ro.observe(this._e);
    }
    sameInteger(t, e) { return Math.round(t) === Math.round(e); }
    hasRectChanged(t) { return !this._lastSize || !t || !(this.sameInteger(t.x, this._lastSize.x) && this.sameInteger(t.y, this._lastSize.y) && this.sameInteger(t.w, this._lastSize.w) && this.sameInteger(t.h, this._lastSize.h)); }
    ...
    render(t, e) {
        ...
        if (s) {
            let i = this._config;
            e && (i = JSON.parse(JSON.stringify(this._config)), i.animate = !1), p(t, s, i, this._animationGroupDelay, this._seed), this._lastSize = s, this._state = "showing";
        }
    }
    size() { return this.sizeFor(this._e.getBoundingClientRect()); }
    sizeFor(t) {
        if (this._svg) {
            const e = this._svg.getBoundingClientRect(),
                s = t;
            return { x: (s.x || s.left) - (e.x || e.left), y: (s.y || s.top) - (e.y || e.top), w: s.width, h: s.height };
        }
        return null;
    }
}

function m(t, e) { return new _(t, e); }

function w(t) {
    let e = 0;
    for (const s of t) {
        const t = s;
        t._animationGroupDelay = e;
        e += 0 === t.animationDuration ? 0 : t.animationDuration || 800;
    }
    const s = [...t];
    return { show() { for (const t of s) t.show(); }, hide() { for (const t of s) t.hide(); } };
}
